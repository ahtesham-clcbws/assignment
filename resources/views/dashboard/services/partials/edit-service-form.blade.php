<section>
    ...
    <form method="post" action="{{ route('service.update', $service->id) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <x-select-input id="location" name="location" class="mt-1 block w-full" required autofocus
                ...
                :defaultOption="$service->location"
                :optionPlaceholder="__('Select location')" :options="$locations" />
        </div>
        <div>
            ...
            <x-text-input id="price" name="price" type="number" class="mt-1 block w-full" required autofocus
                :value="$service->price" autocomplete="price" />
        </div>
        <div>
            <x-input-label for="image" :value="__('Change Service Image')" />
            <x-text-input id="image" name="image" accept=".jpg, .jpeg, .png" type="file"
                class="mt-1 block w-full" autofocus autocomplete="image" />
        </div>
        <div>
            ...
            <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" required
                :value="$service->description" autofocus autocomplete="description" />
        </div>
